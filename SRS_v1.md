...
| S-OUT-07 | 운영 모니터링 체계 (PagerDuty, Slack 연동, API latency/5xx 에러율 대시보드) | Phase 2 (사용자 규모 확대 후) |
| S-OUT-08 | A/B 테스트·코호트 분석 등 실험 검증 프레임워크 (Validation Plan) | Phase 2 (Closed Beta 이후) |
| S-OUT-09 | 유지보수성 강화 (증권사 어댑터 패턴, 모듈 응집도 검증) | Phase 2 (CSV → API 전환 시) |
...
| 클라이언트 | 플랫폼 | 설명 |
|---|---|---|
| 웹 앱 (반응형) | 브라우저 (Chrome, Safari, Firefox, Edge) | MVP v1.3 유일 클라이언트. 모바일 반응형 지원. |
...
* **REQ-NF-001**: 대시보드 렌더링을 위한 API p95 응답 시간은 500ms 이하를 유지합니다.
* **REQ-NF-002**: 알람 감지 로직 처리 및 이메일 발송 큐 적재 시간은 데이터 파싱 완료 후 1초 이내여야 합니다.
* **REQ-NF-004**: **CSV 파싱 및 정규화 데이터 DB 적재 처리 시간은 5초 이내여야 합니다.** (건수 1,000건 기준)
* **REQ-NF-006**: Gemini API를 이용한 복기 일지 생성은 호출부터 응답 저장까지 30초 이내에 완료되어야 합니다.

#### 4.2.2 확장성 (Scalability)
* **REQ-NF-009**: **초기 테스터 50명, 동시접속 10명 이내 환경**에서 안정적이고 빠른 퍼포먼스를 보장해야 합니다. 서버리스 구조(Vercel)의 장점을 활용해 트래픽 변화에 유연하게 대응합니다.

#### 4.2.3 가용성 / 신뢰성 (Availability / Reliability)
* **REQ-NF-011**: MVP 단계의 가용성 목표는 **'Best Effort'** 로 운영하며, Vercel/Supabase Pro 플랜의 SLA를 상속받습니다.
* **REQ-NF-012**: 데이터 파싱 간 유실률(오류 제외 순수 누락)은 0%여야 합니다.
* **REQ-NF-015**: Gemini 프롬프트 응답에 따른 JSON 파싱 실패율은 재시도 로직을 포함해 2% 미만으로 통제합니다.
...
#### 4.2.6 비즈니스 KPI (MVP 최소 측정 기준)

> MVP 단계에서도 핵심 가치 검증을 위해 다음 지표를 추적합니다. 고도화된 실험 설계 및 통계적 검증은 Phase 2(Closed Beta 이후)에서 수행합니다.

* **REQ-NF-033**: 북극성 지표 — WAU 중 **주간 뇌동매매 0회 유저 비율 ≥ 40%** 를 목표로 합니다. (측정: 업로드 기반 unplanned_trade_count 집계)
* **REQ-NF-034**: D7 리텐션 — 가입 후 7일 시점 재방문율 **≥ 35%** 를 목표로 합니다. (측정: Supabase Auth 로그인 이벤트 기반)
* **REQ-NF-036**: 평균 복기 시간 — Gemini 기반 자동 복기 도입 후 **≤ 3분 (p50)** 으로 단축을 목표로 합니다.
...
| NFR - 비즈니스 KPI (MVP 최소) | REQ-NF-033, 034, 036 | TC-NF-033, 034, 036 | Must |
...
### 6.4 Phase 2 이관 항목 (MVP 이후 로드맵)

> 아래 항목은 v1.1에서 존재했으나 MVP 다운사이징 과정에서 의도적으로 제외한 사항입니다. 초기 테스터(50명) 피드백 및 Closed Beta 결과에 따라 Phase 2에서 순차 도입합니다.

| 영역 | 이관 항목 | 도입 조건 |
|---|---|---|
| 운영 모니터링 | API p95 latency, 5xx 에러율, 알람 실패율 대시보드 (PagerDuty/Slack 연동) | 사용자 100명 이상 달성 시 |
| 실험 검증 | A/B 테스트(알람 ON/OFF), 코호트 분석(복기 사용 여부), Van Westendorp 가격 조사 | Closed Beta 종료 후 |
| 증권사 API 연동 | 한국투자·토스·LS REST API 자동 동기화 (CSV 수동 업로드의 상위 호환) | CSV UX 만족도 < 70% 시 |
| 유지보수 강화 | 증권사 어댑터 패턴, Server Actions 응집도 검증, 모듈 재사용성 극대화 | API 연동 도입 확정 시 |
| 보안 고도화 | 분기 침투 테스트, ISMS 갱신 심사, RBAC 관리자/사용자 권한 분리 | 유료 구독 100명 이상 |

> **문서 끝** — SRS-001 v1.3 | ISO/IEC/IEEE 29148:2018 준수 | Vibe Coding MVP Version
