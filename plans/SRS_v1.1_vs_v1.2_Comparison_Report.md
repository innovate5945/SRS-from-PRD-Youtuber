# SRS v1.1 vs v1.3 변경 사항 비교 리포트

- **비교 대상:** `SRS_v0_1_Opus.md` (v1.1) ↔ `SRS_v1_2.md` (v1.3)
- **작성일:** 2026-05-02 (v1.3 개선 반영)
...
> v1.1은 상용급 인프라(증권사 REST API 연동, Vercel Cron, 자체 매칭 로직 등)를 전제했으나, v1.3에서는 **바이브 코딩(AI 코드 생성 100% 의존)에 최적화된 기술 스택**으로 전면 재조정되었습니다.

| 항목 | v1.1 (SRS_v0_1_Opus.md) | v1.3 (SRS_v1_2.md) | 변경 근거 |
|---|---|---|---|
...
> v1.1은 **SOM 5,000명 대상 상용 서비스 수준(SLA 99.5%)**을 목표로 했으나, v1.3은 **초기 테스터 50명 대상의 현실적 MVP**로 목표를 재설정했습니다. 핵심 가치(감정 제어, 자동 복기)는 유지하면서 전달 방식만 변경합니다.
...
| **비즈니스 KPI** | REQ-NF-033~037 (5건) | WAU 뇌동매매 0회 비율, D7 리텐션, 무료→유료 전환, 복기 시간, NPS | **핵심 3건 복원** (REQ-NF-033, 034, 036) — 북극성·리텐션·복기시간. 전환율·NPS는 Phase 2 이관 | ⚠️ v1.3에서 개선 — MVP에서도 최소 KPI 추적 보장 |
...
| 항목 | v1.1 | v1.2 | 비고 |
|---|---|---|---|
| **문서 분량** | 961줄 (45KB) | 553줄 (28KB) | **42% 축소** — MVP 다운사이징 + Phase 2 이관 항목 명시 |
...
| **Traceability Matrix** | Story 단위 22행 (세부 AC별 매핑) | Feature 단위 **9행** (범위 기반 그룹 매핑 + KPI 행 추가) | 추적 단위를 Story→Feature로 추상화 |
...
| 섹션 | v1.1 | v1.3 | 비고 |
|---|---|---|---|
| §4.2.6 운영/모니터링 (Observability) | REQ-NF-026~030, PagerDuty/Slack 연동 | **삭제** → Out-of-Scope(S-OUT-07) + §6.4 Phase 2 이관에 명시 | ⚠️ v1.3에서 이관 추적 보장 |
| §4.2.7 유지보수성 (Maintainability) | REQ-NF-031~032 | **삭제** → Out-of-Scope(S-OUT-09) + §6.4 Phase 2 이관에 명시 | ⚠️ v1.3에서 이관 추적 보장 |
| §4.2.8 비즈니스 KPI | REQ-NF-033~037 (5건) | **핵심 3건 복원** (§4.2.6 비즈니스 KPI + Traceability 추적) | ⚠️ v1.3에서 개선 |
| §6.4 Validation Plan | A/B 테스트, 코호트 분석, 가격 조사 | **삭제** → Out-of-Scope(S-OUT-08) + **§6.4 Phase 2 이관 항목 신설** | ⚠️ v1.3에서 로드맵 명시 |
...
| **기타** | 문서 961줄 → 553줄 (42% 축소), API 22건 → 9건, 클래스 10개 → 5개, 데이터 엔터티 7개 → 6개로 전반적 다운사이징. |

---

## v1.3 개선 사항 (총평 기반 후속 조치)

> 아래는 v1.1 ↔ v1.2 비교 총평에서 식별된 개선 포인트를 **v1.3에서 반영**한 내역입니다.

| # | 총평에서 식별된 문제 | v1.3 적용 내용 |
|---|---|---|
| 1 | 모니터링·KPI·실험 검증이 "MVP 이후 이관"이라고만 언급되고, SRS 문서 내에 이관 추적 근거 없음 | **Out-of-Scope에 S-OUT-07~09 추가** (모니터링·Validation Plan·유지보수) + **§6.4 Phase 2 이관 항목 테이블 신설** (5개 영역별 도입 조건 명시) |
| 2 | 비즈니스 KPI가 전체 삭제되어 MVP에서도 핵심 가치 검증이 불가능 | **§4.2.6 비즈니스 KPI 섹션 복원** — 북극성(뇌동매매 0회 ≥40%), D7 리텐션(≥35%), 평균 복기 시간(≤3분)의 핵심 3건 추적 + Traceability 행 추가 |
| 3 | NFR §4.2.1~4.2.3 영역에 평어체(~한다/~이다) 잔존 | **전체 경어체 통일** (~합니다/~됩니다) — 문서 톤 일관성 확보 |
| 4 | 문서 리비전이 v1.2에서 멈춰 개선 이력 미추적 | **v1.2 → v1.3 리비전 업데이트** (Revision, 제목, In-Scope, 문서 끝 모두 반영) |
